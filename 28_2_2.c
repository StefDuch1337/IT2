...
int main(void)
{
    ...
    while (1)
    {
        ...
        {              
            zahl.nachK += 1.0 * nachKommaZwischenSpeicher / pow(10.0, strlen(nachKommaStr)); // Umwandlung in Kommazahlen um dann aufzuaddieren
            if (zahl.nachK >= 1)    //Übertrag aufrechnen
            { // Übertrag
                zahl.vorK++;
                zahl.nachK--;
            }
